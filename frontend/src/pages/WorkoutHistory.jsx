...
export default function WorkoutHistory() {
  ...
  // Inline Quick Add Exercise State
  const [addingExerciseSessionId, setAddingExerciseSessionId] = useState(null);
  ...
  const handleSubmit = async (e) => {
    ...
    // Filter out rows where exerciseName is empty
  ...
  const handleAddExerciseToSession = async (e, sessionId) => {
    e.preventDefault();
    if (!newExName.trim()) return;

    try {
      await axios.post(`${API_URL}/workout/sessions/${sessionId}/exercises`, {
        exerciseName: newExName,
        sets: parseInt(newExSets) || 0,
        reps: parseInt(newExReps) || 0,
        weight: parseFloat(newExWeight) || 0,
        rpe: newExRpe ? parseInt(newExRpe) : null,
        note: newExNote,
      });
      ...
      setNewExRpe('');
      setNewExNote('');
      setAddingExerciseSessionId(null);

      // Reload
      loadSessions();
    } catch (error) {
      alert(error.response?.data?.message || 'Gagal menambahkan gerakan');
    }
  };
          ...
          {sessions.map((sess) => (
            ...
            >
              ...
              <div className="mt-4 pt-4 border-t border-card-border/60">
                {addingExerciseSessionId === sess.id ? (
                  <form onSubmit={(e) => handleAddExerciseToSession(e, sess.id)} className="space-y-3 bg-slate-50 p-4 border border-card-border rounded-2xl">
                    <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider mb-2">Tambah Gerakan Baru ke Sesi Ini</h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
                      <div className="md:col-span-2">
                        <label className="block text-[10px] font-bold text-text-muted mb-1">Nama Gerakan</label>
                        <input
                          type="text"
                          required
                      ...
                      <div>
                        <label className="block text-[10px] font-bold text-text-muted mb-1">Set</label>
                        <input
                          type="number"
                          required
                          min="1"
                      ...
                      <div>
                        <label className="block text-[10px] font-bold text-text-muted mb-1">Repetisi</label>
                        <input
                          type="number"
                          required
                          min="1"
                      ...
                      <div>
                        <label className="block text-[10px] font-bold text-text-muted mb-1">Beban (kg)</label>
                        <input
                          type="number"
                          step="0.1"
                          required
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-2">
                      <div>
                        <label className="block text-[10px] font-bold text-text-muted mb-1">RPE (1-10)</label>
                        <input
                          type="number"
                          min="1"
                          max="10"
                      ...
                      <div className="sm:col-span-2">
                        <label className="block text-[10px] font-bold text-text-muted mb-1">Catatan</label>
                        <input
                          type="text"
                          placeholder="Opsional (misal: RPE 8, terasa berat)"
                          value={newExNote}
                    ...
                    <div className="pt-2 flex justify-end gap-2 text-xs">
                      <button
                        type="button"
                        onClick={() => setAddingExerciseSessionId(null)}
                        className="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-800 font-bold rounded-xl"
                      >
                        Batal
                      </button>
                      <button
                        type="submit"
                        className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-750 text-white font-bold rounded-xl shadow-md"
                      >
                        Simpan Gerakan
                      </button>
                    </div>
                  ...
                  <button
                    onClick={() => {
                      setAddingExerciseSessionId(sess.id);
                      setNewExName('');
                      setNewExSets(3);
                      setNewExReps(10);
                      setNewExWeight(20);
                    ...
                    className="flex items-center gap-1.5 py-2 px-3.5 bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl transition-all"
                  >
                    <Plus className="w-3.5 h-3.5 text-indigo-600" />
                    <span>Tambah Gerakan Ke Sesi Ini</span>
                  </button>
                )}
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full block min-w-0 appearance-none px-4 py-2.5 bg-white border border-card-border rounded-xl text-text text-sm focus:outline-none focus:border-indigo-650"
                  />
                </div>
                <div className="space-y-1.5">
